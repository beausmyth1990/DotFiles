...
@test "wbe | exact file name specified with exact and partial match | entry is inserted into file with exact name match" {
  local workbook1="test_$(uuidgen)"
  local workbook2="test_$(uuidgen)"

  touch ~/workbook/"$workbook1"
  touch ~/workbook/"$workbook2"

  run wbe $workbook2 "some entry..."

  ! [[ -e ~/workbook/"test" ]]
  [[ $(grep -e "^$(date '+%Y-%m-%d')$" <~/workbook/"$workbook2" | wc -l) -eq 1 ]]
  [[ $(grep "$(date '+%H:%M'):" <~/workbook/"$workbook2" | wc -l) -eq 1 ]]

  rm ~/workbook/"$workbook1"
  rm ~/workbook/"$workbook2"
}
